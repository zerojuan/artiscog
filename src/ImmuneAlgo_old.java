...
//		System.out.println();
    ...
    private void clonAlg(){
    	//for i=0 to selectionSize, call mutate( antibodyPool.elementAt(i), i)
    	//trim();
    }
    
    private void mutate(AntiBody AB, int rank){
    	//for i=0 to numClones, mutate and add to antibodyPool
    }
...
//      		System.out.println("The new list is: " + antibodyPool);
    ...
    private double affinity(String antibody){
//    	System.out.print("n:"+numVertices+" v:"+countViolated(antibody)+" c:"+countColors(antibody));
    	double returned=(double)numVertices/((double)Math.pow(countViolated(antibody)+1,2)+(double)countColors(antibody));
    	return returned;
    }
    ...
    //	System.out.println("rank: "+i+" populationSize: "+populationSize+" clonal factor: "+clonalFactor+" numXms: "+numChromosomes+" clones: "+numClones+" gene: "+gene);
    	...
    	for(int counter=0; counter<numClones; counter++){
    		...
    		AB.setAffinity(affinity(mutant));
    		antibodyPool.add(AB);
...
//    	System.out.println(numVertices+"#");
    	for(int i=0; i<number; i++){
    		...
    		tempAB.setAffinity(affinity(tempString));
